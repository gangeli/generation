...
public class Hypergraph<Widget> {
  ...
  public void errIfNonEmpty(Object node) {
    if(!nodes.containsKey(node) || numEdges(node) == 0) { throw new RuntimeException("" + node + " has no children hyperedges (it's empty)"); }
  }
  public boolean isNodeEmpty(Object node){
    return numEdges(node) > 0;
  }
...
//      if(edge == null) throw new IllegalStateException("Null edge: in NodeInfo: " + nodeInfo);
//      if(edge.dest1 == null) throw new IllegalStateException("Null dest1 for node " + nodeInfo);
//      if(edge.dest2 == null) throw new IllegalStateException("Null dest12 for node " + nodeInfo);
